...
parser.add_argument('--catalogfile', dest='catalogfile', type=str, default='/o9000/MWA/GLEAM/hetu_images/deep_learn/inference_sets', help='catalog file')
...
objn = re_csv['objectname'].values


#pro_arr = np.array_split(range(len(objn)),num_process)
for n in range(len(objn)):#pro_arr[rank]:
    items = objn[n]
    ...
    print(parameters)
    try:
       res = requests.post('https://third.ucllnl.org/cgi-bin/firstcutout', data=parameters, stream=False, timeout=20)
