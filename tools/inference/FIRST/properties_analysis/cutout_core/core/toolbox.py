...
def get_sexagesimal_string(position):
    ...
    # truncated = format(np.round(float(sexcoords[0][:9]), decimals=1),'0>8.1f')+sign+format(np.round(float(sexcoords[1][:9]), decimals=1),'0>8.1f') # agreed upon cirada sig dig for name: J033200.0+050000.0
    c1 = SkyCoord(ra=position.ra.value, dec=position.dec.value, unit=(u.deg, u.deg), frame='fk5')
