...
for i in range(len(ra)):#pro_arr[rank]:
    if label[i] == cln: 
       ...
       xw = x2 - x1
       yw = y2 - y1
       r = np.max([xw, yw]) *1.8/3600.0/2.0 #in deg, FIRST pixel resolution: 1.8 arcsec 
       try:
           ...
           #change to use box length for search radius
           result_table = Ned.query_region(co, radius=r * u.deg, equinox='J2000.0')# J2000
           #print(result_table)
