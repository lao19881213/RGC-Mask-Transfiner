...
class SurveyABC(ABC):
    ...
    def process_tile_group(self, tiles, position, size, group, index):
        ...
        if len(tiles) > 1:
            ...
            # only label if not mosaicked UNLESS by date obs should all be same epoch
            if survey_name == "VLASS" and group[0].isdigit():
                fits_data['epoch'] = self.get_epoch(tiles[0][1])

            # Updating individual tiles
            for (tile, tile_url) in tiles:
                # Adding signature
                self.add_CIRADA_signature(tile)

                # Adding filename headers to their respective tiles (for RACS files)
                if survey_name == 'RACS':
                    filename = re.search("RACS-DR.*fits", tile_url).group(0)
                    if filename:
                        filename = filename.replace('%3F', '&').replace('%2B', '+').replace('?', '&')
                        tile.header['FILNAM'] = filename
            ...
            # only label if not mosaicked for now in case multiple epochs
            if survey_name == "VLASS":
                fits_data['epoch'] = self.get_epoch(fits_data['filename'])

            # Adding filename header to files with a single tile
            if survey_name == "RACS":
                filename = re.search("RACS-DR.*fits", tiles[0][1]).group(0)
                if filename:
                    filename = filename.replace('%3F', '&').replace('%2B', '+').replace('?', '&')
                    cutout.header['FILNAM'] = filename
