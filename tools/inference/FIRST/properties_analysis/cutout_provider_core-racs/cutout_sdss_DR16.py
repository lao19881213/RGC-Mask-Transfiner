...
ras = hetu_data['RA_ICRS'].values
decs = hetu_data['DE_ICRS'].values
...
for m in range(len(labels)):
        for cln in clns.keys():
            ...
            if(labels[m]==cln):
               #print(cln)
               position = coordinates.SkyCoord(ra=ras[m], dec=decs[m], unit=(u.deg, u.deg), frame='icrs')
               #try:               
               ...
               #band='ugriz' 
               imgs = astroSDSS.get_images(matches=result_hetu, band='ugriz',  data_release=16)
