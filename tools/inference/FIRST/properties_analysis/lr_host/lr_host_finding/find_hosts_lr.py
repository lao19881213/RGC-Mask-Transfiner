...
from astropy.table import Table, join, hstack, vstack, MaskedColumn, unique
...
def subset_required_wise_images(catalog, wise_meta, acol='RA', dcol='DEC',
    ...
    catpos = SkyCoord(ra=catalog[acol], dec=catalog[dcol], unit=u.deg)
...
#    ((1.08574/hosts['e_W1mag'])>5)
    ...
    for err in irmagerrs:
        ...
        if type(iroptdata[err])!=MaskedColumn:
           iroptdata[err] = MaskedColumn(iroptdata[err])
        ...
        if already_done == True:
            ...
            if len(s_data)>0 and len(h_data)>0: ##ensures data in both sub catalogs
                ...
                if len(catdata['mw'][(catdata['mw']['Mask']==0)])>0 and len(catdata['radio'])>0:
                    ...
                    catdata['radio'].write(rfile, overwrite=True)
                    catdata['mw'].write(hfile, overwrite=True)
                    fake_im.writeto(ifile, overwrite=True)
