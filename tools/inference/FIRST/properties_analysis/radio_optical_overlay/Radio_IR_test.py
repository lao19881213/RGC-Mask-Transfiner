...
#radio image
#fig = plt.figure() 
hdu= fits.open('FIRST_J153149.9+113421.7.fits')[0]
data = hdu.data[:,:]
img = hdu.data[:,:]
...
#IR or Optical image
f=aplpy.FITSFigure('WISE_J153149.9+113421.7_w1.fits', slices=[0], north=True)
hdu_IR= fits.open('WISE_J153149.9+113421.7_w1.fits')[0]
data_IR = hdu_IR.data
#print(data_IR.shape)
#f.show_grayscale() 
#f.show_colorscale(vmin=np.percentile(data_IR,5),vmax=np.percentile(data_IR,50), cmap='plasma')#,stretch='log')
f.show_colorscale(cmap='gray_r',stretch='log')
...
f.recenter(232.95779,11.572684254914623, radius=0.009)
...
#f.save()
f.save('%s.png' % 'J153149.92+113425.0')
