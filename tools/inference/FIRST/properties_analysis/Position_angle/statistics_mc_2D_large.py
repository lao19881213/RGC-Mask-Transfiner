...
def Sn_vs_n(tdata,Sn_mc,Sn_data,filename,angular_radius,ending_n=180):
	...
	try:
		...
		#axarr[1].set_ylabel('sigma')
                ...
                if parallel_transport:
                        plt.savefig('./figures/2D_large/SL_vs_n_PT'+filename+'.pdf')#,overwrite=True)
                else:
                        plt.savefig('./figures/2D_large/SL_vs_n_'+filename+'.pdf')#,overwrite=True)
		...
		if parallel_transport:
			plt.savefig('./figures/2D_large/SL_vs_n_PT'+filename+'.png',overwrite=True)
		else:
			plt.savefig('./figures/2D_large/SL_vs_n_'+filename+'.png',overwrite=True)
...
def angular_radius_vs_n(tdata,filename,n=180,starting_n=2):
	...
	#plt.title('Angular radius vs n for ' + filename)
        ...
        plt.savefig('./figures/2D_large/angular_radius_vs_n_'+filename+'.pdf') #,overwrite=True)
