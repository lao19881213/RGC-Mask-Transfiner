...
hetu_csv = hetu_csv[hetu_csv['z'].notnull()]
print('len of redshift data -->', len(hetu_csv))
RPAs = hetu_csv['RPA'].values
ras = hetu_csv['RA'].values
decs = hetu_csv['DEC'].values

flux_FIRST = hetu_csv['S1.4'].values
...
for mm in range(len(RPAs)):
    ...
    if ras[mm] >= 90 and ras[mm] <= 270:
       ...
       Fluxs.append(flux_FIRST[mm])
    else:
       RPAs_S.append(RPAs[mm])

#print('Max flux in sp data ->', np.max(np.array(Fluxs)))
#Fluxs = np.array(Fluxs)
#print(len(Fluxs[Fluxs>500]))
print('final len ->', len(RPAs_N))
fig = plt.figure()
ax1 = plt.gca()
...
print(ax.get_rmax())
ax.text(np.radians(-25),ax.get_rmax()/2.,r'Number ($\times 10^3$)',
...
#plt.show()
ax.margins(0)
plt.savefig('RPAs_polar_redshift.pdf')
