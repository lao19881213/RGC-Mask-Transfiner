...
hetu_csv = hetu_csv[hetu_csv['f_z']=='s']
print('len of sp redshift data -->', len(hetu_csv))
RPAs = hetu_csv['RPA'].values
ras = hetu_csv['RA'].values
decs = hetu_csv['DEC'].values

print('Max flux in sp data ->', np.max(hetu_csv['S1.4'].values))
...
#plt.show()
ax.margins(0)
plt.savefig('RPAs_polar_redshift_sp.pdf')
