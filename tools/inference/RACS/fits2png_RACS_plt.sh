...
do
    #for snr in $SNRS;
    #do
    if [ ! -d "${IMG_DIR}/${fn}_split_png" ];then
       mkdir -p ${IMG_DIR}/${fn}_split_png
    else
       echo "${IMG_DIR}/${fn}_split_png is already exists"
    fi
    echo "Processing ${fn} ... ..."

    python fits2png_RACS_plt.py --inpdir ${IMG_DIR}/${fn}_split_fits --outdir ${IMG_DIR}/${fn}_split_png 
    #done
done
