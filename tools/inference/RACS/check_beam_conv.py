...
input_dir = '%s/part%d_conv' % (data_dir, args.partid)
...
for fn in file_nms:
    ...
    ratio_final.append(hdr['BMAJ']/hdr['BMIN']) 
beaminfo = pd.DataFrame({'SBID':SBID_final,'BMAJ':bmaj_final,'BMIN':bmin_final,'BPA':bpa_final,'Ratio':ratio_final})

beaminfo.to_csv("%s/part%d_beam_conv.csv" % (data_dir, args.partid),index=False,sep=',')    
