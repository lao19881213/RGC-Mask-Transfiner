...
montage_path = '/home/software/Montage/cpu/Montage/bin'
...
SDC1_HOME = '/p9550/MWA/GLEAM/blao/hetu_images/deep_learn/sdc1/data'
...
def cutout_training_image(cat_csv, field_fits):
    ...
    os.system(splitimg_cmd % (field_fits, out_fname, x1, y1, (x2 - x1), (y2 - y1)))
...
def gen_ds9_region(cat_csv, fits_img, pb, consider_psf=True, fancy=True):
    ...
    with open(cat_csv, 'r') as fin:
        lines = fin.read().splitlines()
        for line in lines[3:]: #skip the header, test the first few only
            fds = line.split(',')
            selected = int(fds[-3])
            if (0 == selected):
                continue
...
if __name__ == '__main__':
    ...
    data_dir = os.path.join(cur_dir, '..', 'SDCdevkit2018')
    train_file = '%s/training_set/TrainingSet_B2_v2_ML.txt' % SDC1_HOME
    train_csv = '%s/training_set/TrainingSet_B2_v2_ML.csv' % SDC1_HOME
    whole_field = '%s/SKAMid_B2_1000h_v3.fits' % SDC1_HOME
    train_field = '%s/SKAMid_B2_1000h_v3_train_image.fits' % SDC1_HOME
    convert_to_csv(train_file)
    cutout_training_image(train_csv, whole_field)
    #pb = '%s/Ancillary_data/PrimaryBeam_B5.fits' % SDC1_HOME
    #gen_ds9_region(train_csv, train_field, pb)
