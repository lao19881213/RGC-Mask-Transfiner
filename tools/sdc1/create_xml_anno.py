...
BMIN = 1.66666679434E-04  #2.53611124208E-05#1.66666679434E-04#4.16666676756E-04
...
CAT_XML_PASCAL_DICT = {'1_1': '1s_1c', '2_1': '2s_1c', '3_1': '3s_1c', \
                        '2_2': '2s_2c', '2_3': '2s_3c', '3_3': '3s_3c'}
...
def _setup_db_pool():
    ...
    return ThreadedConnectionPool(1, 3, database='sdclbq', user='postgres')
...
def build_fits_cutout_index(fits_cutout_dir,
    ...
    if cur.fetchone()[0] :
       ...
       sqlStr = """CREATE TABLE {0}
                   (fileid TEXT    NOT NULL,
                    coverage           sbox    NOT NULL);"""
       cur.execute(sqlStr.format(tablename))
...
def draw_gt(cat_csv, split_fits_dir, split_png_dir, table_name, pb, target_dir, fancy=True):
    ...
    faint_sources = 0
    for idx, line in enumerate(lines[3:]): #skip the header, test the first few only
        # if (idx == 15000):
        #     break
        if (idx % 1000 == 0):
            print("scanned %d" % idx)
        ...
        total_flux = float(lll[5])
        total_flux = _primary_beam_gridding(total_flux, ra, dec, pb_wcs, pb_data)
        ...
        sqlStr = "select fileid from %s where coverage ~ scircle " % table_name +\
                    ...
                    #"(spoint '(%fd, %fd)', %f);" % (ra, dec, max(pixel_res_x, pixel_res_y))
        cur = conn.cursor()
        cur.execute(sqlStr)
        res = cur.fetchall()
        if (not res or len(res) == 0):
            print("Fail to find fits for source {0}".format(lll[0]))
            continue
        print("idx: %s, res0: %s" % (idx,res[0])) 
        for fd in res:#res:
            fid = fd[0] #fd[0]
...
def create_tree(imgdir, image_name, h, w):
    ...
    # create primary branch folder
    folder = SubElement(annotation,'folder')
    # add folder label content
    folder.text=(imgdir)

    # create primary branch filename
    filename=SubElement(annotation,'filename')
    filename.text=image_name

    # create primary branch path
    path=SubElement(annotation,'path')

    path.text= '{}/{}'.format(imgdir,image_name)  # return image work dir
    
    # create primary branch source
    ...
    #create barnch segmented
    segmented = SubElement(annotation,'segmented')
    segmented.text = '0'

GT_Source = namedtuple('GT_Source', ['box', 'objectname'])

def prepare_xml(cat_csv, split_fits_dir, split_png_dir, table_name, pb, fancy=True):
    pb_wcs, pb_data = _setup_pb(pb)
    ...
    for idx, line in enumerate(lines[3:]): #skip the header, test the first few only
        ...
        total_flux = float(lll[5])
        total_flux = _primary_beam_gridding(total_flux, ra, dec, pb_wcs, pb_data)
        avg_image_flux = _calc_image_flux(total_flux, major, minor, ra, dec, pb_wcs, pb_data)
        ...
        for fd in res:#res:
            ...
            gs = GT_Source(box=(x1, y1, x2, y2), objectname=CAT_XML_PASCAL_DICT[combo])
            xml_dict[fid].append(gs)
    ...
    return images, xml_dict

def create_xml(xml_dict, png_dir):
    for k, v in xml_dict.items():
        k = k.replace('.fits', '.png')
        png_fn = osp.join(png_dir, k)
        im = cv2.imread(png_fn)
        h, w, _ = im.shape
        create_tree(png_dir, png_fn, h, w)

        for cs in v:
            bbox = cs.box
            labeln = cs.objectname
            if int(round(bbox[0])) ==0:
               bbox = (1, bbox[1], bbox[2], bbox[3])
            if int(round(bbox[1])) == 0 :
               bbox = (bbox[0], 1, bbox[2], bbox[3])
            create_object(annotation, int(round(bbox[0])), int(round(bbox[1])), int(math.ceil(bbox[2])), int(math.ceil(bbox[3])), labeln)
        xml_tree = tostring(annotation, pretty_print=True) #formation, line breaks 
        #tree = ET.ElementTree(annotation)   
        k = k.replace('.png', '.xml')    
        xml_fn = osp.join(png_dir, k)
        with open(xml_fn, 'wb') as xml_ob:
             xml_ob.write(xml_tree)
        #tree.write(xml_fn)
...
if __name__ == '__main__':
    ...
    #parser.add_argument('--rank', dest='rank', type=int, default='0', help='rank')
